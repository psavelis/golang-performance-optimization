...
// Profiling-enabled version of the optimized generator
func main() {
	// CPU profiling
	cpuProfile, err := os.Create("generator_optimized_cpu.prof")
	if err != nil {
		panic(err)
	}
	...
	// Memory profiling
	defer func() {
		memProfile, err := os.Create("generator_optimized_mem.prof")
		if err != nil {
			panic(err)
		}
